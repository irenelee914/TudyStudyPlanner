...
class ViewController: UIViewController  {
    ...
    @IBAction func addNewAssignment(_ sender: UIBarButtonItem) {
        ...
        let button = EKProperty.ButtonContent(label: buttonText, backgroundColor: .flatPurple, highlightedBackgroundColor: .flatBlue) {
            ...
            newAssignment.tag = textField2.output
            newAssignment.dueDate = textField3.output
            newAssignment.startDate = textField4.output
            
            let temp = Float(textField5.output) ?? 0
            if temp == 0 {
                
                
                
                Drop.down("Error: Incorrect weighting input, please try again", state: .error)
                
                
                
            }
            else {
                newAssignment.weighting = temp
                self.saveAssignment(assignment: newAssignment)
